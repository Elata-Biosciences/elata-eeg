...
fn test_voltage_conversion_mathematics() {
    ...
    assert!(
        ...
        "Full scale positive should be ~{}V, got {}V",
        expected,
        voltage
    );
    ...
    assert!(
        (voltage - expected).abs() < 0.0001,
        "Full scale negative should be ~{}V, got {}V",
        expected,
        voltage
    );
    ...
    assert!(
        (lsb_voltage - expected_lsb).abs() < 0.000001,
        "LSB should be {}V, got {}V",
        expected_lsb,
        lsb_voltage
    );
}
...
fn test_gain_scaling_behavior() {
    ...
    assert!(
        (voltage_gain1 / voltage_gain6 - 6.0).abs() < 0.01,
        "Gain 1 vs 6 ratio should be ~6x"
    );
    assert!(
        (voltage_gain1 / voltage_gain12 - 12.0).abs() < 0.01,
        "Gain 1 vs 12 ratio should be ~12x"
    );
    assert!(
        (voltage_gain1 / voltage_gain24 - 24.0).abs() < 0.01,
...
fn test_precision_and_rounding() {
    ...
    assert!(
        (diff - expected_step).abs() < 0.000001,
...
fn test_typical_eeg_signal_ranges() {
    ...
    assert!(
        (calculated_voltage - target_voltage).abs() < 0.000001,
    ...
    assert!(
        (calculated_noise - noise_voltage).abs() < 0.0000001,
