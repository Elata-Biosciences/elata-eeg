...
# Remove service unit files if present and reload systemd
...
if [ "${REMOVED_ANY:-0}" = "1" ]; then
    sudo systemctl daemon-reload && print_success "Systemd configuration reloaded" || print_warning "Failed to reload systemd"
else
    print_info "No service unit files found to remove."
fi
